  ...
  return (
    <Tag ref={ref} className={className} style={{ display: 'flex', flexWrap: 'wrap' }}>
      {elements.map((segment, index) => {
        ...
        // Support markdown-like *italic* wrapping for em tags
        const isItalic = segment.startsWith('*') && segment.endsWith('*');
        const cleanSegment = isItalic ? segment.slice(1, -1) : segment;
        ...
        return (
          ...
          >
            {isItalic ? (
              <em>{cleanSegment === ' ' ? '\u00A0' : cleanSegment}</em>
            ) : (
